'use strict';
const fs = require('./fsPromise'), path = require('path'), os = require('os'), cp = require('child_process');
//load linters
let lintBuilders = {};
...
class Linter {
    ...
    _exeLinter(svc, cmdOpts) {
        return new Promise((resolve, reject) => {
            ...
            if (cmdOpts.data)
                spawned.stdin.end(cmdOpts.data);
        })
            .catch(e => ({
            error: e,
            linter: svc.title || svc.exe,
    ...
    _runLinters() {
        ...
        const final = result => this.cb(result)
            ...
            if (this.linting[result.linter].pend)
                this._runLinters();
            if (result.error)
                this._runLinters();
        });
        ...
        if (needsTmpFile) {
            ...
            executor.then(() => Promise.all(noTmpFileSvc()));
