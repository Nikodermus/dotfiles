...
class SassConvertService {
    ...
    _checkSassConvert() {
        try {
            let sassConvertVersion = child_process_1.execSync(`${this.sassConvertCommand} --version`).toString('utf8').trim();
    ...
    setSassConvertCommand() {
        let configurationSassPath = vscode_1.workspace.getConfiguration('sassFormat').get("sassPath");
