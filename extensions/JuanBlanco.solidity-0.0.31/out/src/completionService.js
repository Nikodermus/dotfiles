...
class CompletionService {
    ...
    createParamsInfo(params) {
        ...
        if (typeof params !== 'undefined' && params !== null) {
            params.forEach(parameterElement => {
                ...
                if (typeof parameterElement.id !== 'undefined' && parameterElement.id !== null) {
    ...
    createFunctionEventCompletionItem(contractElement, type, contractName) {
        let completionItem = vscode_languageserver_1.CompletionItem.create(contractElement.name);
    ...
    getAllCompletionItems(documentText, documentPath) {
        if (this.rootPath !== 'undefined' && this.rootPath !== null) {
            const contracts = new contractsCollection_1.ContractCollection();
            contracts.addContractAndResolveImports(documentPath, documentText, projectService.initialiseProject(this.rootPath));
            let completionItems = [];
...
function GetCompletionTypes() {
    ...
    types.forEach(type => {
        let completionItem = vscode_languageserver_1.CompletionItem.create(type);
...
function GeCompletionUnits() {
    ...
    etherUnits.forEach(unit => {
        let completionItem = vscode_languageserver_1.CompletionItem.create(unit);
