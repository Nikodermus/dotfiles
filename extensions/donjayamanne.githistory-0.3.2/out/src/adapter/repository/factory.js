...
let GitServiceFactory = class GitServiceFactory {
    ...
    createGitService(workspaceRoot) {
        const id = md5(workspaceRoot);
        if (!this.gitServices.has(id)) {
            this.gitServices.set(id, new git_1.Git(this.serviceContainer, workspaceRoot, this.gitCmdExecutor, this.logParser, this.gitArgsService));
        }
