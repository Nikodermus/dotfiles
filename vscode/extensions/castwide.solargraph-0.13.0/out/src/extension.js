...
const YardContentProvider_1 = require("./YardContentProvider");
const RubyCompletionItemProvider_1 = require("./RubyCompletionItemProvider");
const RubySignatureHelpProvider_1 = require("./RubySignatureHelpProvider");
const RubyHoverProvider_1 = require("./RubyHoverProvider");
const RubyDefinitionProvider_1 = require("./RubyDefinitionProvider");
...
const solargraphContentProvider = new YardContentProvider_1.default(solargraphServer);
...
function initializeAfterVerification(context) {
    solargraph.updateGemDocumentation(solargraphConfiguration);
    solargraphServer.start().then(function () {
        prepareWorkspace();
    });
    if (isCodeCompletionEnabled()) {
        context.subscriptions.push(vscode.languages.registerCompletionItemProvider(['ruby', 'erb'], new RubyCompletionItemProvider_1.default(solargraphServer), '.', '@', '$'));
    }
    if (isIntellisenseEnabled()) {
        context.subscriptions.push(vscode.languages.registerSignatureHelpProvider(['ruby', 'erb'], new RubySignatureHelpProvider_1.default(solargraphServer), '(', ')'));
        context.subscriptions.push(vscode.languages.registerHoverProvider(['ruby', 'erb'], new RubyHoverProvider_1.default(solargraphServer)));
        context.subscriptions.push(vscode.languages.registerDefinitionProvider(['ruby', 'erb'], new RubyDefinitionProvider_1.default(solargraphServer)));
    }
